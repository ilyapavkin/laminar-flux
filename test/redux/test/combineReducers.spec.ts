...
describe('Utils', () => {
    describe('combineReducers', () => {
        ...
        // Following test excluded due to inaccessibility of internal redux ActionTypes via public API.
        // While it makes sense to test this, it introduce conflict with laminar-flux part.
        /*it('throws an error on first call if a reducer attempts to handle a private action', () => {
            const reducer = combineReducers({
                counter(state: number, action) {
                    console.log('validation', ActionTypes.INIT, action);
                    switch (action.type) {
                        case 'increment':
                            return state + 1
                        case 'decrement':
