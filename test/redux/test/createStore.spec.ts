...
describe('createStore', () => {
    it('exposes the public API', () => {
        ...
        // So we filter it out
        const methods = Object.keys(store) //.filter(key => key !== $$observable) // FIXME: enable this back
    ...
    it('throws if reducer is not a function', () => {
        // This is actually correct in context of laminar-flux.
        // Store with undefined reducer will be created with default one.
        // expect(() => createStore(undefined)).toThrow()
        expect(() => createStore(undefined)).not.toThrow()
    ...
    it('applies the reducer to the previous state', () => {
        const store = createStore(reducers.todos)
        // in this test case it's actually correct to expect combined state, instead of declared in original test
        // passed reducer is mounted inside `LFPipeline`
        expect(store.getState()).toEqual([])
        // expect(store.getState()).toEqual(createStoreContent({ global: [] }))
    ...
    it('accepts enhancer as the third argument', () => {
        ...
        const spyEnhancer = vanillaCreateStore => (...args) => {
            // This line makes no sense in laminar-flux, reducer is wrapped
